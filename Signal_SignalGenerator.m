...
switch flag_sigIn
    case 'gen'
        [sigIn, ~, Config] = OFDM_Mod_g('20MHz','QPSK','off','NR',fnum,[]);
        fs = Config.fs
        bwInband = (Config.bwChannel/2-1e6)*[-1 1]
        bwChannel = Config.bwChannel
        Nsamps = numel(sigIn)
        df = fs/Nsamps
        signal = sigIn;
%         save('waveform_OFDM_20MHz_23p04MHz.mat','signal')
    case 'lod'
...
sigIn_ch = conv(sigIn, b_ch, 'same');
PLOT_FFT_dB_g(sigIn_ch, fs, Nsamps, ['sigIn + channel filter'], 'df', 'full', 'pwr', [fnum+2]);
[evm, delay] = dsp_evm_timexcorr_inband_g(sigIn, circshift(sigIn_ch,57), fs, bwInband, 1, 2)

%% input: digital gain/pOut assignment
PdB_expected = -15
v_expected = sqrt(10.^((PdB_expected)/10));
v_signal = sqrt(mean(abs(sigIn_ch).^2));
...
[sigOut, DelayFilt, LengthFilt, DelayFiltSum, sigOutCell] = radon_interpolation_g1(sigIn_ch_gain, fs, fsOut, bCell, 0, fnum+5);
...
if flag_removeDelay
    ...
    length = numel(sigIn)*ipo_ratio(1)*ipo_ratio(2)*ipo_ratio(3)
    sigOut_rmd = sigOut_o([1:length]+delay);
    PLOT_FFT_dB_g(sigOut_rmd, fsOut, numel(sigOut_rmd), ['sigOut remove delay'], 'df', 'full', 'pwr', [fnum+6]);
end

%% check: EVM
% Nsamps
Nsamps_sigIn = numel(sigIn)
Nsamps_sigIn == (floor( ((numel(sigOut)-DelayFilt(3,3))/2-DelayFilt(2,3))*3/4 ) - DelayFilt(1,3))/2
...
sigOut_DNS1_UPS = interp1( [1:3:3*numel(sigOut_DNS1)], sigOut_DNS1, [1:1:3*numel(sigOut_DNS1)], 'linear');
sigOut_DNS1_UPS = interp1( [1:3:3*numel(sigOut_DNS1)], sigOut_DNS1, [1:1:3*numel(sigOut_DNS1)], 'spline');
sigOut_DNS1_UPS = interp1( [1:3:3*numel(sigOut_DNS1)], sigOut_DNS1, [1:1:3*numel(sigOut_DNS1)], 'v5cubic');
sigOut_DNS1_UPS = interp1( [1:3:3*numel(sigOut_DNS1)], sigOut_DNS1, [1:1:3*numel(sigOut_DNS1)], 'makima');

sigOut_DNS1_UPS_DNS2 = sigOut_DNS1_UPS(1:4:end).';
sigOut_DNS1_UPS_DNS2_DNS3 = sigOut_DNS1_UPS_DNS2(1:2:end);
[sigOut_DNS1_UPS_DNS2_DNS3_shift, DelayInt, DelayFrac] = dsp_Delay_EstCor_g2(sigIn, sigOut_DNS1_UPS_DNS2_DNS3, fnum+7);
sigOut_DNS1_UPS_DNS2_DNS3_shift_trunk = sigOut_DNS1_UPS_DNS2_DNS3_shift(1:Nsamps_sigIn);
[evm] = dsp_evm_g(sigIn, sigOut_DNS1_UPS_DNS2_DNS3_shift_trunk)
...
sigIn_1C = sigOut_rmd;
disp_title = '2C'
[~, sigOut_2C, ~, ~, ~] = SYM_NCOApp(sigIn_1C, fNCO, fsOut, bwInband, flag_SUMCarrier, fnum+8, disp_title, 0);
